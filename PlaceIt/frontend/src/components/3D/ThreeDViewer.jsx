...
            });
            
            // --- FIX START: Scale model to fit the room ---
                            
            // Define the available space in the room (from createRoom function).
            const roomWidth = 25;
            const roomDepth = 25;
            // Add some padding so the furniture doesn't touch the walls.
            const padding = 0.9; 
            const maxModelWidth = roomWidth * padding;
            const maxModelDepth = roomDepth * padding;

            // Calculate the initial size of the model.
            const initialBox = new THREE.Box3().setFromObject(model);
            const modelSize = initialBox.getSize(new THREE.Vector3());

            // Determine the required scale factor to fit the model, maintaining aspect ratio.
            const scaleX = modelSize.x > maxModelWidth ? maxModelWidth / modelSize.x : 1;
            const scaleZ = modelSize.z > maxModelDepth ? maxModelDepth / modelSize.z : 1;
            const scale = Math.min(scaleX, scaleZ);

            // Apply the uniform scale if the model is too large.
            if (scale < 1) {
                model.scale.set(scale, scale, scale);
            }

            // After scaling, re-calculate the bounding box to correctly center and position the model.
            const scaledBox = new THREE.Box3().setFromObject(model);
            const center = scaledBox.getCenter(new THREE.Vector3());
            
            // Reposition the model to be centered in the room and sit on the floor.
            model.position.x -= center.x;
            model.position.y -= scaledBox.min.y; // Use the new, scaled box's minimum Y.
            model.position.z -= center.z;

            // --- FIX END ---

            furnitureGroup.add(model);
            // Pass the ref to store the initial camera state for the now correctly-sized model
